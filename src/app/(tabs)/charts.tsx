...
export default function ChartsScreen() {
  ...
  const pieData = useMemo(() => {
    ...
    if (fixedPaid > 0) {
      map['Contas Fixas'] = { label: 'Contas Fixas', value: fixedPaid, color: '#f97316' }
    }
  ...
  const barData = useMemo(() => {
    return months.map(m => {
      const inMonth = transactions.filter(t => t.date >= m.from && t.date <= m.to)
      return {
        label:   m.label,
      ...
      <View>
        <Svg width={CHART_W} height={280}>
          <G>
            ...
            {/* Texto central */}
            <SvgText x={CX} y={CY - 12} textAnchor="middle" fontSize="11" fill="#9ca3af">
              {selected !== null ? pieData[selected].label : 'Total'}
            </SvgText>
            <SvgText x={CX} y={CY + 10} textAnchor="middle" fontSize="15" fontWeight="bold" fill="#111827">
              {selected !== null
            ...
            {selected !== null && (
              <SvgText x={CX} y={CY + 28} textAnchor="middle" fontSize="11" fill="#6b7280">
        ...
        <View style={s.legend}>
          {pieData.map((d, i) => (
            ...
            >
              ...
              <Text style={s.legendAmount}>{formatCurrency(d.value, currency)}</Text>
            </TouchableOpacity>
          ))}
        </View>
      </View>
  ...
  function renderBar() {
    const BAR_H    = 200
    const BAR_W    = 28
    const GAP      = (CHART_W - barData.length * BAR_W * 2 - 32) / (barData.length + 1)
    const PADDING  = 16
      ...
      <View>
        ...
        <Svg width={CHART_W} height={BAR_H + 40}>
          ...
          {[0, 0.25, 0.5, 0.75, 1].map((pct, i) => {
            ...
            return (
              <G key={i}>
                <Line x1={PADDING} y1={y} x2={CHART_W - PADDING} y2={y}
          ...
          {barData.map((d, i) => {
            ...
            const incH   = d.income  > 0 ? Math.max((d.income  / barMax) * availH, 4) : 0
            const expH   = d.expense > 0 ? Math.max((d.expense / barMax) * availH, 4) : 0

            return (
              <G key={i}>
                {/* Barra de receita */}
                <Rect
                  x={x} y={PADDING + availH - incH}
                  width={BAR_W} height={incH}
                  fill="#22c55e" rx={4}
                />
                {/* Barra de despesa */}
                <Rect
                  x={x + BAR_W + 2} y={PADDING + availH - expH}
                  width={BAR_W} height={expH}
                  fill="#ef4444" rx={4}
                />
                {/* Label do mês */}
                <SvgText
                  x={x + BAR_W} y={BAR_H + 20}
    ...
    <SafeAreaView style={s.safe}>
      ...
      <View style={s.toggle}>
        <TouchableOpacity
          ...
          onPress={() => { setChartType('pie'); setSelected(null) }}
        >
          <Ionicons
            name="pie-chart"
            size={16}
          ...
          />
          <Text style={[s.toggleText, chartType === 'pie' && s.toggleTextActive]}>
            Pizza
          </Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[s.toggleBtn, chartType === 'bar' && s.toggleActive]}
          onPress={() => { setChartType('bar'); setSelected(null) }}
        >
          <Ionicons
            name="bar-chart"
            size={16}
          ...
          />
          <Text style={[s.toggleText, chartType === 'bar' && s.toggleTextActive]}>
            Barras
          </Text>
        </TouchableOpacity>
      </View>
      ...
      >
        ...
        {pieData.length === 0 && (
          <View style={s.empty}>
            ...
            <Text style={s.emptySubtext}>Adicione transações para ver os gráficos</Text>
          </View>
        )}
      </ScrollView>
    </SafeAreaView>
...
const s = StyleSheet.create({
  ...
  toggle:     { flexDirection: 'row', margin: 20, backgroundColor: '#e5e7eb', borderRadius: 12, padding: 4 },
  ...
  subtitle:   { fontSize: 12, color: '#9ca3af', textAlign: 'center', marginBottom: 16 },

  chartBox:   { backgroundColor: '#fff', borderRadius: 20, padding: 16, shadowColor: '#000', shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.06, shadowRadius: 8, elevation: 2 },

  // Legenda pizza
  legend:        { marginTop: 8, gap: 6 },
  legendItem:    { flexDirection: 'row', alignItems: 'center', gap: 10, padding: 10, borderRadius: 10 },
  ...
  // Barras
  barLegend:     { flexDirection: 'row', gap: 20, justifyContent: 'center', marginBottom: 12 },
  ...
  // Resumo meses
  monthSummary: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, marginTop: 16 },
  monthCard:    { flex: 1, minWidth: 80, backgroundColor: '#f8fafc', borderRadius: 10, padding: 10, alignItems: 'center' },
